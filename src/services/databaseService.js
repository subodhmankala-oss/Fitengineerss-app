...
const databaseService = {
  ...
  // ─── CREATE WORKOUT (coach/client-authored plan, separate from the
  // detailed set-logging plans saveWorkoutPlan above handles) ───
  //
  // exercises here follow the workout_templates/workout_drafts shape —
  // {name, sets, reps, rest, order} with sets/reps/rest as plain
  // target numbers/strings, NOT the {name, sets: [{weight, reps, ...}]}
  // per-set logging shape normalizePlanExercises enforces above. Deliberately
  // bypasses saveWorkoutPlan/normalizePlanExercises rather than reusing them,
  // since that filter would silently drop every exercise here (its `sets`
  // is a count, not an array).
  //
  // Sent via restInsert (real session bearer token) so the new
  // workout_plans_insert RLS policy — user_id = self, or is_my_client(user_id)
  // AND coach_id = self — actually has a real auth.uid() to check against.
  async createWorkoutPlan({ userId, coachId, planName, category, workoutType, exercises, createdBy }) {
    if (createdBy !== 'coach' && createdBy !== 'client') {
      throw new Error('createWorkoutPlan requires an explicit createdBy of "coach" or "client".');
    }
    if (!userId) throw new Error('createWorkoutPlan requires the client userId this plan belongs to.');
    if (!Array.isArray(exercises) || exercises.length === 0) {
      throw new Error('This plan has no exercises — add at least one before saving.');
    }
    ...
    const planRecord = {
      user_id: userId,
      // A client-authored plan never carries a coach_id, even if the client
      // has an assigned coach — matches the RLS policy's own assumption
      // (coach_id is only ever set by the coach themselves).
      coach_id: createdBy === 'coach' ? (coachId || null) : null,
      plan_name: planName,
    ...
    if (createdBy === 'coach' && !planRecord.coach_id) {
      throw new Error('createWorkoutPlan requires a coachId when createdBy is "coach".');
    }

    const inserted = await restInsert('workout_plans', planRecord);

    // Best-effort super-admin notification — never let a push/log hiccup
    // fail the actual save the coach/client is waiting on.
    try {
      await fetch('/api/push?action=notify-user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          event: 'workout_created_pending_media',
          clientUserId: userId,
          planName: planRecord.plan_name,
          category: planRecord.category,
          workoutType: planRecord.workout_type,
          creatorRole: createdBy,
          creatorName: localStorage.getItem('userName') || null
        })
      });
    } catch (e) {
      console.warn('workout_created_pending_media notification failed (non-fatal):', e);
    }

    return inserted;
  },

  // ─── SUPER-ADMIN MEDIA REVIEW QUEUE ───
  // SELECT policy already grants is_super_admin() full read of workout_plans
  // (sql/lock_down_reads.sql, untouched by this feature), so a plain
  // restSelect under the admin's own session token is enough — no service-
  // role API route needed.
  async getPendingMediaWorkoutPlans() {
    const rows = await restSelect(`workout_plans?select=*&media_status=neq.completed&order=created_at.desc`);
    return (Array.isArray(rows) ? rows : []).map(p => ({
      ...
      mediaScheduledAt: p.media_scheduled_at
    }));
  },

  // Resolve a handful of users.id -> {id, full_name, email} for display —
  // used by AdminWorkoutReviewQueue to show which coach/client owns each
  // pending plan without a dedicated join endpoint.
  async getUsersByIds(ids) {
    const unique = Array.from(new Set((ids || []).filter(Boolean)));
    if (unique.length === 0) return {};
    const rows = await restSelect(`users?select=id,full_name,email&id=in.(${unique.map(encodeURIComponent).join(',')})`);
    const map = {};
    (Array.isArray(rows) ? rows : []).forEach(u => { map[u.id] = u; });
    return map;
  },

  // Schedule a later pass to add media — sets media_scheduled_at and flips
  // media_status to 'scheduled'. Relies on workout_plans_update's
  // is_super_admin() clause (sql/supabase_create_workout_admin_review.sql).
  async scheduleWorkoutMediaReview(planId, scheduledAtIso) {
    return restUpdate(`workout_plans?id=eq.${encodeURIComponent(planId)}`, {
      media_status: 'scheduled',
      media_scheduled_at: scheduledAtIso
    });
  },

  // Attach an uploaded media URL to one exercise (by index) within the
  // plan's exercises jsonb, or to the whole plan (plan-level `media` column)
  // when exerciseIndex is not a valid index (e.g. -1 / 'plan').
  async attachWorkoutMedia(planId, plan, { exerciseIndex, mediaUrl, mediaType }) {
    const isPerExercise = Number.isInteger(exerciseIndex) && exerciseIndex >= 0
      && Array.isArray(plan.exercises) && exerciseIndex < plan.exercises.length;

    if (isPerExercise) {
      const exercises = plan.exercises.map((ex, idx) => {
        if (idx !== exerciseIndex) return ex;
        const media = Array.isArray(ex.media) ? [...ex.media] : [];
        media.push({ url: mediaUrl, type: mediaType || 'image' });
        return { ...ex, media };
      });
      return restUpdate(`workout_plans?id=eq.${encodeURIComponent(planId)}`, { exercises });
    }

    const planMedia = Array.isArray(plan.media) ? [...plan.media] : [];
    planMedia.push({ url: mediaUrl, type: mediaType || 'image' });
    return restUpdate(`workout_plans?id=eq.${encodeURIComponent(planId)}`, { media: planMedia });
  },

  // Mark this plan's media as done — the super-admin has finished attaching
  // everything it needs.
  async markWorkoutMediaCompleted(planId) {
    return restUpdate(`workout_plans?id=eq.${encodeURIComponent(planId)}`, {
      media_status: 'completed'
    });
  },

  // Raw Storage REST upload into the workout-media bucket, sent with the
  // caller's real session bearer token (not the bare anon key) — the
  // workout_media_insert storage policy checks is_super_admin(), which needs
  // a real auth.uid() to resolve. Same request shape as uploadExerciseVideo
  // above, minus that older function's anon-key write model.
  async uploadWorkoutMedia(file) {
    ...
    try {
      const res = await fetch(`${supabaseUrl}/storage/v1/object/workout-media/${fileName}`, {
        method: 'POST',
        headers: {
          apikey: supabaseAnonKey,
          Authorization: `Bearer ${token}`,
    ...
    return `${supabaseUrl}/storage/v1/object/public/workout-media/${fileName}`;
  },
